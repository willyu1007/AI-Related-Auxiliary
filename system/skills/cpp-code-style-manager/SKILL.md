...
本仓库的唯一开发来源是 `<repo-root>/system/skills/cpp-code-style-manager/`。运行时以当前加载的 `SKILL.md` 所在目录为 `<skill-root>`；`.skills-manager` 是外部全局 Skill 管理器，`.codex/skills/cpp-code-style-manager` 若存在则是安装后的链接视图，不是第二份源码。
