...
Read the canonical goal and every `Done when` condition from `01-status.md`; use legacy `00-overview.md` only when the canonical file is absent. Inspect delivered code and the exact task-trailer commit timeline. Read `verification.md` for the latest decisive evidence, falling back to legacy `04-verification.md` only when needed. Check `00-roadmap.md` for unresolved decisions and `proposed:<slug>` follow-ups whose disposition would be lost on archive.
