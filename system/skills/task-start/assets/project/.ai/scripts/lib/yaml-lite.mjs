/** Dependency-free parser for the YAML subset emitted by project governance. */
...
function _unquoteScalar(s) {
  ...
  if ((t.startsWith('"') && t.endsWith('"')) || (t.startsWith("'") && t.endsWith("'"))) {
    return _decodeQuotedScalar(t);
  }
  return t;
}

function _decodeQuotedScalar(value) {
  if (value.startsWith('"')) {
    try {
      return JSON.parse(value);
    } catch (error) {
      throw new Error(`yaml-lite: invalid double-quoted scalar: ${error?.message || String(error)}`);
    }
  }
